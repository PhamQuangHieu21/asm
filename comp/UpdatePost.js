...
const UpdatePost = (props) => {
  //
  const [content, setContent] = useState(props.route.params.item_post.content)
  ...
  const getDropDown_Update = async () => {
    let from_api = 'http://10.24.32.47:80/Categories';
  ...
  const updateSP = () => {
    ...
    let url_api_update = 'http://192.168.1.15:80/Posts/' + id_post;
  ...
  return (
    <View style={styles.khung_modal}>
      ...
      <TextInput placeholder='Nhập nội dung bài viết' style={styles.text_input_update}
        onChangeText={(txt) => { setContent(txt) }} value={content} />
